...
let larget = [...numbers].sort((a, b) => a - b).pop();
...
let strinsLength = strings.map((string) => string.length);

// - Find the sum of the length of words using above question
strinsLength.reduce((acc, cv) => {
  acc = acc + cv;
  return acc;
}, 0);
// - Customers Array
...
let filteedCustomer = customers.filter((customer) => customer.firstname.startsWith("J")); 
